...
public class SafeExecutor : ISafeExecutor
{
        ...
        Guid? userId = null)
    {
        ...
        {
            await _logDispatcher.Value.Dispatch(logEvent, reason: reasonOnFailure, errorStack: pgEx.Message, email: email,
                tenantId: tenantId, userId: userId);
            return SafeExecutionResult.Fail(errorCode, _localizer.GetErrorMessage(errorCode), _userContext.TraceId);
        }
        catch (Exception ex)
        {
            await _logDispatcher.Value.Dispatch(logEvent, reason: reasonOnFailure, errorStack: ex.Message, email: email);
            return SafeExecutionResult.Fail(errorCode, _localizer.GetErrorMessage(errorCode), _userContext.TraceId);
        }
    }
